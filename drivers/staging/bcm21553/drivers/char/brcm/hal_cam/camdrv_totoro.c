...
{
	int rc;
	//printk(KERN_INFO "CAMDRV_GetRegulator enter");

	cam_regulator_i = regulator_get(NULL, "cam_vddi");
	if (!cam_regulator_i || IS_ERR(cam_regulator_i)) {
		printk( "cori[cam_vddi]No Regulator available");
		rc = -EFAULT;
		return rc;
	}
	...
	if (!cam_regulator_a || IS_ERR(cam_regulator_a)) {
		printk( KERN_INFO"cori[cam_vdda]No Regulator available");
		rc = -EFAULT;
		return rc;
	}
	cam_regulator_c = regulator_get(NULL, "cam_vddc");
	if (!cam_regulator_c|| IS_ERR(cam_regulator_c)) {
		printk( KERN_INFO"cori[cam_vddc]No Regulator available");
		rc = -EFAULT;
		return rc;
	}
	return 1;
...
static int CAMDRV_TurnOnRegulator()//CYK_TEST
{
	int rc=1;
	unsigned long stat;
	//printk(KERN_INFO"Enter CAMDRV_TurnOnRegulator");
	rc = CAMDRV_GetRegulator();
	rc += CAMDRV_SetRegulator();
	printk(KERN_INFO"Regulator test 0");
#if 1
  //	regulator_set_voltage(cam_regulator_i,1800000,1800000);	
	if (cam_regulator_i){
		rc = regulator_enable(cam_regulator_i);
		printk(KERN_INFO"Regulator test 1");
	}
	//regulator_set_voltage(cam_regulator_a,2800000,2800000);	
	
	if (cam_regulator_a){
		rc = regulator_enable(cam_regulator_a);
		printk(KERN_INFO"Regulator test 2");
	}
	//regulator_set_voltage(cam_regulator_c,1800000,1800000);
	
	if (cam_regulator_c){
		rc = regulator_enable(cam_regulator_c);	
		printk(KERN_INFO"Regulator test 3");	
	}
#endif
