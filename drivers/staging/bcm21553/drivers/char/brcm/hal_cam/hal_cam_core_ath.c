...
static wait_queue_head_t gVideoReadyQ;
	...
	case CAM_IOCTL_GET_FRAME:
		{
			...
			if(Drv_Scene==CamSceneMode_Auto) //check for only test mode
            {
               ...
               {
                   printk("Video frame timeout, err!\n");
                   return -2;
               }
            }
	...
	case CAM_IOCTL_GET_JPEG_AND_THUMBNAIL:
		{
			...
			if (copy_from_user
				...
				HalcamTraceDbg("CAM_IOCTL_GET_JPEG_AND_THUMBNAIL CHECK JPEG STATUS START \n");

#ifdef CONFIG_BCM_CAM_ISX005
				CAMDRV_CheckJpegStatus();	
#endif		
				HalcamTraceDbg("CAM_IOCTL_GET_JPEG_AND_THUMBNAIL CHECK JPEG STATUS END \n");
				...
				HalcamTraceDbg("CAM_IOCTL_GET_JPEG_AND_THUMBNAIL CHECK JPEG PROCESS FRAME END \n");
				...
				HalcamTraceDbg("CAM_IOCTL_GET_JPEG_AND_THUMBNAIL CHECK JPEG PROCESS DONE \n");
			}
			break;
		}
	...
	case CAM_IOCTL_GET_STILL_YCbCr_BufferVA:
		{
            ...
            HalcamTraceDbg("[Albert] __va(capture_buffer_global) = 0x%x \n",__va(capture_buffer_global));
            HalcamTraceDbg("[Albert] capBufVA = 0x%x \n",capBufVA);
...
static int process_frame(CamSensorSelect_t sensor, unsigned char* jpegBuf, unsigned char * thumBuf)
{
	...
	/* Block waiting to be signalled by either VSYNC ISR or camera disable */
		if (0 == wait_event_interruptible(gDataReadyQ, c->still_ready)) {
				HalcamTraceDbg( "process_frame started \n");
			c->still_ready = 0;
 ...
 * The disable method wakes up with gProcessFrameRunning = 0 which means exit */
			...
			HalcamTraceDbg( "process_frame before interleave data \n");
			...
			HalcamTraceDbg( "process_frame after interleave data \n");
...
static void init_queue(struct buf_q *queue)
{
	if (queue) {
		...
		queue->Num = 0;
		HalcamTraceDbg("Queue RD and WR %d and %d\n",queue->ReadIndex,queue->WriteIndex);
	} else {
	}
	/*
	   g_CurrData.id =-1;
						...
						UInt32 length)
{
	...
	for (i = 0; i < length / sizeof(CamSensorIntfCntrl_st_t); i++) {
		...
		default:
			HalcamTraceDbg("CNTRL - Not Supported\n");
...
#if 1 //move power up sequence to camdrv
...
static int cam_power_down(CamSensorSelect_t sensor)
{
   ...
   /*     if (cam_g->cam_regulator_i) //move power down sequence to camdrv
...
#if 1 //move power down sequence to camdrv
...
int camera_enable(CamSensorSelect_t sensor)
{
	...
	if ((c->main.mode == CamStillnThumb) || (c->main.mode == CamStill)) {
		...
		if(c->main.format == CamDataFmtJPEG)
		{
            ...
            HalcamTraceDbg("Start address for still JPEG image capture is 0x%x\n",cslCamBuffer1.start_addr);
		}
		else
		{
		...
		cslCamBuffer1.line_stride = sensor_size.resX * 2 ;
			...
			HalcamTraceDbg("Start address for still YUV image capture = 0x%x, base address = 0x%x \n",cslCamBuffer1.start_addr,cam_g->cam_buf.phy);
		}
		...
		if (csl_cam_set_image_type_control(c->hdl, &cslCamImageCtrl ) ){
			HalcamTraceErr(KERN_ERR"Unable to set image type control\n");
		}
		}
		/* PIPELINE CONTROL */
		...
		if(csl_cam_rx_start(c->hdl))
			HalcamTraceDbg("Unable to start RX\n");
...
static int cam_open(struct inode *inode, struct file *file)
{
	...
	//board_sysconfig(SYSCFG_CAMERA,SYSCFG_INIT); //aska disable
...
//		gpio_direction_output(CAM_FLASH_EN,1);
	...
	{
	    HalcamTraceDbg("[CAM] Flash OFF [%s] \n", __func__);
	}
