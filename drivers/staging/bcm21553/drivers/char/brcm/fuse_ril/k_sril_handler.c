    ...
    switch(pdata->handler_state)
    {
        ...
        case BCM_RESPCAPI2Cmd:
        {
            ...
            else
            {
                ...
                {
                    rdata->type = 2;
                    /* prevent fix : comparing unsigned less than zero is never true */
/*
...
*/                    
                            ...
                            USSD_RELEASE_COMPLETE_RETURN_ERROR == rsp->ussd_data.service_type)
                {
                    ...
                    if( rsp->ussd_data.err_code == SUPPSVCSTATUS_UNKNOWN_ALPHABET ) {
                        memset(rdata->USSDString, 0, PHASE1_MAX_USSD_STRING_SIZE+1);
                        rdata->Length = 1;
                        rdata->codetype = 0x47;
                        KRIL_DEBUG(DBG_INFO, "USSD_RELEASE_COMPLETE_RETURN_ERROR - err_code : SUPPSVCSTATUS_UNKNOWN_ALPHABET");
                        pdata->handler_state = BCM_FinishCAPI2Cmd;
                        return;
                    }
                }
...
void KRIL_SRIL_GetPhoneBookEntryHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch(pdata->handler_state)
    {
		...
		if((int)alpha_data->alpha_size> (PB_ALPHA_MAX/2)-1)
		{
			alpha_data->alpha_size = (PB_ALPHA_MAX/2)-1;
			memset(ril_rsp->name, 0, (PB_ALPHA_MAX/2));
		}
		else
			...
			case ALPHA_CODING_UCS2_80:
				...
				if(temp_length > (PB_ALPHA_MAX/2)-1 )
					temp_length = (PB_ALPHA_MAX/2)-1 ;
...
//Email
	if((pbk_etc_rec->num_of_email >= 1 ) && (pbk_etc_rec->email[0].alpha_size > 0) &&( rsp->pbk_id ==PB_3G_GLOBAL))
		...
		if((int)email_data->alpha_size> (PB_EMAIL_MAX/2)-1)
		{
			email_data->alpha_size = (PB_EMAIL_MAX/2)-1;
			memset(ril_rsp->email, 0,(PB_EMAIL_MAX/2));
		}
		else
		...
		if( email_valid )
		{
			switch(email_data->alpha_coding)
				...
				if(temp_length > (PB_EMAIL_MAX/2)-1 )
					temp_length = (PB_EMAIL_MAX/2)-1 ;
    ...
    switch(pdata->handler_state)
    {
		...
		if(rsp_result->usim_adn_info_exist)
		{
			for (i = 0; i < MAX_3GPP_TYPE ; i++)
			{
				...
				if(ril_rsp->response[i][0]  ==  0x04 )	//email
				{
					if (pbk_config[i].entryLength > 50 )	//HJKIM_50
						ril_rsp->response[i][2] = 50;
					else
				...
				if((i==0)&&((int)rsp_result->mx_alpha_size >40))
					ril_rsp->response[i][2]=	40;//HJKIM_50
