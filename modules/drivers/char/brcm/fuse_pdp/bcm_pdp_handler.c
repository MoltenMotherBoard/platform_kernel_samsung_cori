...
static void FillDataResponseTypeApn(UInt8 cid, char* type, char* apn)
{
    ...
    for (i=0; i<BCM_NET_MAX_DUN_PDP_CNTXS; i++)
    {
        if (cid == Kpdp_pdp_resp[i].cid)
        {
            ...
            if (NULL != apn) {
		strncpy(Kpdp_pdp_resp[i].apn, apn, PDP_APN_LEN_MAX - 1);
	    	Kpdp_pdp_resp[i].apn[PDP_APN_LEN_MAX - 1] = '\0';
	    }
...
static void FillDataResponseAddress(UInt8 cid, char* address)
{
    ...
    for (i=0; i<BCM_NET_MAX_DUN_PDP_CNTXS; i++)
    {
	    ...
	    {
		// not null-terminated
		memcpy(Kpdp_pdp_resp[i].address, address, PDP_ADDRESS_LEN_MAX);
	    }
...
void KPDP_SetupPdpHandler(void *pdp_cmd, Kpdp_CAPI2Info_t *capi2_rsp)
{
    ...
    {
        KPDP_DEBUG(DBG_ERROR,"CAPI2 response is NULL. handler_state:%d\n", pdata->handler_state);
