    ...
    switch(pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            if(!pdata->bcm_ril_rsp){
                KRIL_DEBUG(DBG_ERROR, "Unable to allocate bcm_ril_rsp memory\n");
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
            }
            else{
        ...
        case BCM_CC_GetCallNumber:
        {
            ...
            KRIL_DEBUG(DBG_INFO, "MSG_CC_GETCALLNUMBER_RSP::rdata->index:%d KRILCallState->phone_number:********\n", rdata->index);
            CAPI2_CC_IsMultiPartyCall(GetNewTID(), GetClientID(), rdata->KRILCallState[rdata->index].index);
            pdata->handler_state = BCM_CC_IsMultiPartyCall;
        }
        break;
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            if (tdata->isVTcall)
            {
                ...
                {
                    KRIL_DEBUG(DBG_ERROR, "Phone number: ********\n");
