...
static netdev_tx_t bcm_fuse_net_tx(struct sk_buff *skb, struct net_device *dev)
{
    void *buff_data_ptr;
...
//CSP#509054. workaround. 
//In atomic context, RPC_PACKET_AllocateBufferEx (INTERFACE_PACKET, skb->len, pdp_cid, PKT_ALLOC_NOWAIT);
//
    //Allocate a buffer
    buffer = RPC_PACKET_AllocateBufferEx (INTERFACE_PACKET, skb->len, pdp_cid, PKT_ALLOC_NOWAIT);

    if(unlikely(buffer == NULL)) 
    {
        netif_tx_stop_queue(dev);       // if Tx function returns NETDEV_TX_BUSY without calling netif_stop_queue() when the IPC queue is full, it will chew tons of CPU.
        BNET_DEBUG(DBG_ERROR,"%s: Error buffer Handle cid %d\n", __FUNCTION__, pdp_cid);
        ndrvr_info_ptr->stats.tx_errors++;
        return NETDEV_TX_BUSY;            // Tx function should return NETDEV_TX_BUSY when queue full so network device scheduler will re-queue the packet into qdisc and re-schedule the Tx after net driver calls netif_wake_queue().
    }
