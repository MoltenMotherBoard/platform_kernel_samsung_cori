...
#include "SILABS_S5300_BL006_ALPS01_APP01.h"
...
int WriteBytes (unsigned int addr, unsigned int num)
{
   ...
   if(TSP_MODULE_ID==0x01)
   for(i=0;i<num;i++) SMB_DATA_OUT[i+9] = ALP_APP01_Binary[addr+i]; 
...
int GetPageCRC (unsigned int addr)
{
   ...
   {
		if(TSP_MODULE_ID==0x01)
			write_data = ALP_APP01_Binary[addr+i];  
